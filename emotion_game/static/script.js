...
let lastProcessedEmotionId = null;
...
// removed duplicate button click sound handler
    ...
    console.log('⌨️ Keyboard shortcuts: 1-4 (options), Ctrl+R (restart), Ctrl+S (settings)');
});

// --- Enhanced Features ---

// removed duplicate playClickSound {
    const audio = document.getElementById('clickSound');  // was wrong ID before
    if (audio) {
        audio.currentTime = 0;
        audio.play().catch(e => console.warn("Click sound error:", e));
    }
...
function startEmotionPolling() {
    ...
    // Check for new emotions every 1 second
    emotionPollingInterval = setInterval(checkForNewEmotion, 1000);
    console.log('Started emotion');
}
...
function checkForNewEmotion() {
    ...
    // Get the latest emotion from the API
    fetch('/api/last_emotion')
        .then(response => response.json())
        .then(data => {
            // Process the emotion if it's new and valid
            if (data && data.emotion && data.id !== lastProcessedEmotionId) {
                processEmotion(data);
                lastProcessedEmotionId = data.id;
            }
        })
        .catch(error => {
...
// Process a detected emotion or emotion
function processEmotion(emotionData) {
    const now = Date.now();

    // Enforce cooldown to prevent rapid emotion selection
    if (now - lastEmotionTime < GESTURE_COOLDOWN) {
        return;
    }
    ...
    // Map emotions/emotions to option indices
    let optionIndex = -1;

    if (currentMode === 'emotions') {
        // Map emotions to option indices
        if (emotion.includes('happy') || emotion.includes('joy')) {
            optionIndex = 0;
        } else if (emotion.includes('sad') || emotion.includes('sorrow')) {
            optionIndex = 1;
        ...
        // Map hand emotions to option indices
        if (emotion.includes('thumbs_up') || emotion.includes('thumbs up')) {
            optionIndex = 0;
        } else if (emotion.includes('thumbs_down') || emotion.includes('thumbs down')) {
            optionIndex = 1;
        } else if (emotion.includes('victory') || emotion.includes('peace')) {
            optionIndex = 2;
        } else if (emotion.includes('pointing') || emotion.includes('pointing up')) {
            optionIndex = 3;
        }
    }
    ...
    if (optionIndex >= 0 && optionIndex < currentOptions.length) {
        showEmotionIndicator(`Detected: ${emotionData.emotion} (Option ${optionIndex + 1})`, 'success');
...
async function loadPassageFromAPI(passageId) {
    ...
    showEmotionIndicator(`Loading passage: ${passageId}`, 'info');
    ...
    try {
        ...
        showEmotionIndicator(`Error loading story: ${error.message}. Please try again.`, 'error');
...
// Helper functions for emotion indicators
function showEmotionIndicator(message, type = 'info') {
    const indicator = document.getElementById('emotion-indicator');
    if (indicator) {
        indicator.textContent = message;
        indicator.className = `emotion-indicator ${type}`;
        indicator.style.display = 'block';
    }
}

function hideEmotionIndicator() {
    const indicator = document.getElementById('emotion-indicator');
    if (indicator) {
...
async function processPassage(passage) {
    if (!passage) {
        ...
        showEmotionIndicator('This path leads into uncharted territory...', 'warning');
        return;
    }
    ...
    // Show loading indicator
    showEmotionIndicator(`Entering: ${passage.name}`, 'info');
...
function selectOption(destination) {
    if (destination) {
        ...
        showEmotionIndicator(`Traveling to: ${destination}`, 'success');
        ...
        showEmotionIndicator('This path leads into uncharted territory...', 'warning');
        setTimeout(hideEmotionIndicator, 3000);
    }
}
...
function setupEventListeners() {
    ...
    document.getElementById('reconnect-button').addEventListener('click', function() {
        playClickSound();
        showEmotionIndicator('Reconnecting emotion/emotion API...', 'info');
        ...
        fetch('/api/reconnect')
            ...
            .then(data => {
                ...
                if (apiConnected) {
                    showEmotionIndicator('API Connected! Emotion control active.', 'success');
                } else {
                    showEmotionIndicator('API Disconnected. Using demo mode.', 'warning');
                }

                setTimeout(hideEmotionIndicator, 2000);
            })
            .catch(error => {
                console.error('Error reconnecting:', error);
                apiConnected = false;
                updateApiStatus();
                showEmotionIndicator('Failed to connect to API. Using demo mode.', 'error');
                setTimeout(hideEmotionIndicator, 2000);
            });
    });
        ...
        if (debugMode) {
            fetch('/api/debug/endpoints')
                ...
                .then(data => {
                    ...
                    showEmotionIndicator('Debug info in console. Press F12 to view.', 'info');
                    setTimeout(hideEmotionIndicator, 3000);
                })
                .catch(error => {
            ...
            showEmotionIndicator('Debug mode disabled', 'info');
            setTimeout(hideEmotionIndicator, 2000);
        }
    });
    ...
    modeSelect.addEventListener('change', function() {
        ...
        showEmotionIndicator(`Switched to ${this.value} mode`, 'info');
    ...
    document.getElementById('test-api-connection').addEventListener('click', function() {
        playClickSound();
        showEmotionIndicator('Testing API connection...', 'info');
        ...
        fetch('/api/status')
            ...
            .then(data => {
                ...
                const statusMessage = apiConnected
                    ? 'API Connected! Emotion control active.'
                    : `API Disconnected: ${data.api_error}`;

                showEmotionIndicator(statusMessage, apiConnected ? 'success' : 'warning');
                setTimeout(hideEmotionIndicator, 3000);
            })
            .catch(error => {
                ...
                apiConnected = false;
                updateApiStatus();
                showEmotionIndicator('Error testing API connection', 'error');
                setTimeout(hideEmotionIndicator, 3000);
            });
    });
}
...
function showEmotionIndicator(message, type = 'info') {
    emotionIndicator.textContent = message;
    emotionIndicator.className = `emotion-indicator show ${type}`;
}
...
function toggleSettings() {
    settingsFrame.classList.toggle('active');
}
...
function updateUIForApiStatus() {
    const apiStatusElement = document.getElementById('api-status');
    const emotionIndicatorElement = document.getElementById('emotion-indicator');

    if (apiConnected) {
        apiStatusElement.textContent = '🟢 Witcher Senses Active – API Connected';
        apiStatusElement.className = 'api-status connected';
        emotionIndicatorElement.innerHTML = `
            <span class="emotion-icon">🧠</span>
            <span class="emotion-text">Emotion control is active. Use your signs, witcher.</span>
        `;
    } else {
        ...
        emotionIndicatorElement.innerHTML = `
            <span class="emotion-icon">💤</span>
            <span class="emotion-text">No connection. Meditate or check your potions (API).</span>
        `;
    }
}
